...
//                sync.registerCallback(&DepthProcessing::BigCallback, this);
        ...
        pub_obj = priv_nh_.advertise<all_msgs::ObjectArray>("ObjectArray", 3);

<<<<<<< HEAD
        svm = SVM::load("/home/atHomeSS18/God-Watcher/workspace/Finalproject/src/perception/src/svm_trained.xml");
...
=======
                svm->load("/home/atHomeSS18/God-Watcher/workspace/Finalproject/src/perception/src/svm_trained.xml");
...
=======
	...
	if(pcok)
	{
		...
		{
                        Mat mask = Mat::zeros(img1_Ptr->image.size(), CV_8UC1);
                        int x = bx->Boxes[i].x + bx->Boxes[i].width / 2;
                        int y = bx->Boxes[i].y + bx->Boxes[i].height / 2;
                for(uint j = bx->Boxes[i].x; j < bx->Boxes[i].x + bx->Boxes[i].width; j++)
	        {
                    for(uint i = bx->Boxes[i].y; i < bx->Boxes[i].y + bx->Boxes[i].height; i++)
	            {
                        point3D = getDepth(img1_Ptr->image, j, i , cx, cy, fx, fy);
	                if(j == x && i == y)
	                {
			...
			obj.cam_pose.header = img1->header;
                        obj.width = bx->Boxes[i].width;
                        obj.length = bx->Boxes[i].height;
                        obj.semantic_confidence = bx->Boxes[i].probability;
			obj.semantic_id = 11;
			img2_Ptr->image.copyTo(dst, mask);

			obj.is_track = true;
            obj.is_target = (bool) (int) predict(dst);
...
>>>>>>> cc9392bc67e44b10c1c80bc4ba0a97209018581e
            {
                for (uint k = ymin; k < ymax; k++)
                {
                    ...
                    if (j == (xmin + xmax) / 2 && k == (ymin + ymax) / 2)
                    {
        ...
        points = vector<geometry_msgs::Point>(pts);
    }
    imgok2 = true;
        ...
        if (Rois.size() > 0)
        {
            ...
            for (int i = 0; i < Rois.size(); i++)
            {
                ...
                else
                {
                    ...
                    obj.id = i;
                    obj.is_track = true;
                    obj.cam_pose.header = _img_rect->header;
                    obj.width = Rois[i].width;
                    obj.length = Rois[i].height;
                    rectangle(dst, Rois[i].tl(), Rois[i].br(), 2, 8, 0);
                    obj.semantic_id = 11;
                    obj.cam_pose.point.x = points[i].x;
                    obj.cam_pose.point.y = points[i].y;
                    obj.cam_pose.point.z = points[i].z;
                    if (labels.at<float>(i, 0) > 0)
                    {
                        putText(dst, "positive", Rois[i].tl(), font, 3, (255, 255, 255), 1);
                        obj.is_target = true;
                    }
                    else {
                        putText(dst, "negative", Rois[i].tl(), font, 3, (255, 255, 255), 1);
                        obj.is_target = false;
                    }
                    objarr.list.push_back(obj);
                }


            } if ( objarr.list.size() > 0)
                pub_obj.publish(objarr);
        }
